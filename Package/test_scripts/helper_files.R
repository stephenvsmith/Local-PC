...
bn.repository <- function(bn.net,  # character scalar of
  ...
  bn <- readRDS(sprintf(paste0(net.dir, "/rds/%s.rds"), tolower(bn.net)))
...
bnlearn2edgeList <- function(bn.net,
  ...
  bn.net.list <- lapply(bn.net, function(x){
    bn <- readRDS(sprintf(paste0(net.dir, "/rds/%s.rds"), tolower(x)))
...
NetSetup <- function(net,vars){
  ...
  gdg <- generate.data.grid(vars$data_grid,
                            out.dir=sim_net_dir,
                            verbose=FALSE,path.start=var_list$ps2)
...
test_bn_setup <- function(net,target,vars){
  ...
  #target <- c(2,3)
  larger_ones <- c('barley','child')
  lmax <- ifelse(net %in% larger_ones,4,3)
  start_time <- Sys.time()
...
# This function will calculate all the wrongly oriented edges (includes case where local PC CPDAG has no orientation when there should be)
    ...
    for (j in 1:ncol(true_cpdag)){
      ...
      if (true_cpdag[i,j] == 1 & true_cpdag[j,i] == 0) {
        if (lpc_cpdag[i,j] == 0 & lpc_cpdag[j,i] == 1){
          total <- total + 1
        } else if (lpc_cpdag[i,j] == 1 & lpc_cpdag[j,i] == 1){
          total <- total + 1
        }
      }
...
local_pc_dist <- function(target,vars){
  ...
  metrics <- c(missingEdges(vars$true_local_cpdag,vars$lpc_amat),
               addedEdges(vars$true_local_cpdag,vars$lpc_amat),
               wrongDirection(vars$true_local_cpdag,vars$lpc_amat),
               bnlearn::shd(vars$cpdag,vars$t_cpdag),
               same_v_structs)
